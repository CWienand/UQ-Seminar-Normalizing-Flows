...
class Multivariate_Diag_Base_MC_mapping(nf.distributions.base.BaseDistribution):
    ...
    def forward(self, num_samples = 1, context = None):
        xi_RQMC = torch.rand((self.n_dims,num_samples)) # generates digitally shifted Sobol points.
...
class Multivariate_Diag_t_qmc(nf.distributions.base.BaseDistribution):
    ...
    def forward_given_samples(self, x, context = None):
        z = torch.stack([torch.tensor(scale*stats.t.ppf(np.array(x[:,i]),self.df)-loc) for i,(loc,scale) in enumerate(zip(self.loc,self.diag))],axis = 1)
        #Sanity check:          
        #print(self.distribution.cdf(z)-torch.tensor(xi_RQMC)) 
        #Should give something close 0 for the shape of the sample!   
        return z, self.log_prob(z)
...
class Multivariate_t_target(nf.distributions.target.Target):
